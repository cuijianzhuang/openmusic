...
<p align="center">
  ...
  <a href="deploy/DEPLOY-BAOTA.md">宝塔部署</a>
</p>
...
管理后台默认 `http://localhost:3000/admin`（`admin` / `admin123`），建议配置红点渠道 Cookie。
...
npm run install:all
cp server/.env.example server/.env   # 编辑必填项
npm run build
...
> 生产环境由**同一 Node 进程**托管 API、WebSocket 与前端静态资源。请保持**单实例**（房间状态在进程内存中）。
...
**后端** Node.js · Express · Socket.IO · Redis（可选）
