...
export default function MyRoomsAccess() {
  ...
  return (
    <>
      <button type="button" onClick={show} className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/[0.04] px-3 py-2 text-sm text-white/70 transition hover:bg-white/[0.08] hover:text-white">
        <DoorOpen className="h-4 w-4" />
        <span className="hidden sm:inline">我的房间</span>
      </button>
      {open && (
        <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/65 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="我的房间">
          <div className="w-full max-w-lg rounded-3xl border border-white/10 bg-[#11131a] p-5 shadow-2xl">
